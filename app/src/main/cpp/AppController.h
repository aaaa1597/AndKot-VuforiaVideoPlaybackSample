...
private: // methods
    ...
    /// Callback for Vuforia to report a runtime error that must be handled.
    /// clientData points to the AppController instance that is going to handle the error.
    static void onEngineError(VuEngineError errorCode, void* clientData);

    /// Called internally by onEngineError on the AppController instance
    /// pointed to by clientData.
...
private: // data members
    /// Callback to inform the user of synchronous Vuforia Engine creation errors
    ErrorMessageCallback mErrorMessageCallback;
    /// Callback to inform the platform code of asynchronous runtime errors
    /// that occur on Vuforia Engine threads
